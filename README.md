...
| Подсистема  | Стратегия    | Фактор репликации | Конфигурация и синхронизация          | Вид синхронизации |
|-------------|--------------|-------------------|---------------------------------------|-------------------|
| Посты       | master-slave | 3                 | hot-standby sync + warm-standby async | Логическая        |
| Реакции     | master-slave | 3                 | hot-standby sync + warm-standby async | Логическая        |
| Комментарии | master-slave | 3                 | hot-standby sync + warm-standby async | Логическая        |
| Подписки    | master-slave | 3                 | hot-standby sync + warm-standby async | Логическая        |
| Поиск       | -            | -                 | -                                     | -                 |
| Фотографии  | master-slave | 2                 | warm-standby async                    | Логическая        |

С такой конфигурацией получим баланс между надежностью хранения и скоростью.

### Шардирование

| Подсистема  | Вид       | Ключ шардирования |
|-------------|-----------|-------------------|
| Посты       | key-based | user_id           |
| Реакции     | key-based | post_id           |
| Комментарии | key-based | post_id           |
| Подписки    | key-based | user_id           |
| Поиск       | -         | -                 |
| Фотографии  | key-based | post_id           |

### Количество хостов
  Выбираем те же типы дисков, что и в системе с одним хостом, по причине цены. 

| Подсистема  | Тип диска | hosts         | hosts with replication |
|-------------|-----------|---------------|------------------------|
| Посты       | nVME      | 3 / 2 = 2     | 2 * 3 = 6              |
| Реакции     | nVME      | 3 / 2 = 2     | 2 * 3 = 6              |
| Комментарии | SSD       | 4 / 2 = 2     | 2 * 3 = 6              |
| Подписки    | SSD       | 2 / 2 = 1     | 1 * 3 = 3              |
| Поиск       | -         | -             |                        |
| Фотографии  | HDD       | 463 / 2 = 232 | 232 * 2 = 464          |
